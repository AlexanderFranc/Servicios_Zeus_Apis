...
namespace Infraestructure.Repository.Core
{
    public class PlanificacionRepository : GenericCoreRepository<ComponentesPlanificacionDto>, IPlanificacionRepository
    {
        ...
        public ComponentesPlanificacionDto getById(int idPlanificacion)
        {
            ComponentesPlanificacionDto planificacion = new ComponentesPlanificacionDto();
            DataSet ds_planificacion = Conexion.BuscarZEUS_ds("PLANIFICACION pln inner join PERIODO per on per.ID_PERIODO=pln.ID_PERIODO inner join MALLA m on m.ID_MALLA = pln.ID_MALLA inner join COMPONENTE cpt on pln.ID_TIPO_COMPONENTE=cpt.ID_SUBTIPO_COMPONENTE and cpt.ID_MATERIA = m.ID_MATERIA and cpt.ID_PLAN_ESTUDIO = m.ID_PLAN_ESTUDIO \r\ninner join MATERIA mat on cpt.ID_MATERIA=mat.ID_MATERIA\r\ninner join PLAN_ESTUDIO ple on cpt.ID_PLAN_ESTUDIO=ple.ID_PLAN_ESTUDIO\r\ninner join MODALIDAD_PE mo on mo.ID_MODALIDAD_PE=pln.ID_MODALIDAD_PLANIFICACION\r\ninner join EMPLEADO pro on pro.IDENTIFICACION_EMP=pln.DNI_PROFESORC\r\ninner join MODALIDAD_PERIODO mope on per.ID_MODALIDAD=mope.ID_MODALIDAD\r\ninner join ESPACIOS_FISICOS ef on ef.ID_ESPACIOS_FISICOS=pln.ID_ESPACIOS_FISICOS inner join NIVEL_INFRAESTRUCTURA ninf on ninf.ID_NIVEL_INFRAESTRUCTURA=ef.ID_NIVEL_INFRAESTRUCTURA\r\ninner join INFRAESTRUCTURA inf on inf.ID_INFRAESTRUCTURA=ninf.ID_INFRAESTRUCTURA \r\ninner join SUBTIPO_COMPONENTE scpt ON pln.ID_TIPO_COMPONENTE = scpt.ID_SUBTIPO_COMPONENTE ", "pln.ID_MALLA,\r\nmat.HORAS_SEMESTRALES_MATERIA,\r\nmat.CREDITOS_MATERIA,\r\nper.ID_PERIODO,\r\nper.CODIGO_PERIODO,\r\nmat.ID_MATERIA,\r\nmat.CODIGO_MATERIA,\r\nmat.NOMBRE_MATERIA,\r\nPARALELO,CUPO,\r\npln.DNI_PROFESORC,\r\npro.NOMBRES_EMP,\r\npro.APELLIDO_EMP,\r\npln.ID_MODALIDAD_PLANIFICACION,\r\nmo.NOMBRE_MODALIDAD_PE,\r\nmope.NOMBRE_MODALIDADP,\r\nef.CODIGO_ESPACIOS_FISICOS, \r\nple.ID_PLAN_ESTUDIO,\r\nple.CODIGO_PLAN_ESTUDIO_MALLA,\r\nID_PLANIFICACION,\r\npln.ID_TIPO_COMPONENTE,\r\npln.ID_PERIODICIDAD_PLANIFICACION,\r\nef.ID_ESPACIOS_FISICOS,\r\nper.ID_MODALIDAD,\r\nper.ID_ESTADO_PERIODO,\r\nef.ID_NIVEL_INFRAESTRUCTURA,\r\ninf.ID_INFRAESTRUCTURA,\r\nscpt.CODIGO_SUBTIPO_COMPONENTE,\r\npln.ACTIVO   ", "where pln.ID_PLANIFICACION=" + idPlanificacion);
            if (ds_planificacion.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds_planificacion.Tables[0].Rows)
                {
        ...
        public List<ComponentesPlanificacionDto> obtenerPlanificacionTH(int idperiodo, int idFacultad) 
        {
            ComponentesPlanificacionDto planificacion = new ComponentesPlanificacionDto();
            List<ComponentesPlanificacionDto> listaPlanificacion = new List<ComponentesPlanificacionDto>();
            DataSet ds_planificacion = Conexion.BuscarZEUS_ds(
                "PLANIFICACION pln \r\ninner join PERIODO per on per.ID_PERIODO=pln.ID_PERIODO \r\ninner join MALLA m on m.ID_MALLA = pln.ID_MALLA inner join COMPONENTE cpt on pln.ID_TIPO_COMPONENTE=cpt.ID_SUBTIPO_COMPONENTE and cpt.ID_MATERIA = m.ID_MATERIA and cpt.ID_PLAN_ESTUDIO = m.ID_PLAN_ESTUDIO \r\ninner join MATERIA mat on cpt.ID_MATERIA=mat.ID_MATERIA\r\ninner join PLAN_ESTUDIO ple on cpt.ID_PLAN_ESTUDIO=ple.ID_PLAN_ESTUDIO\r\ninner join CARRERA carr on ple.ID_CARRERA=carr.ID_CARRERA\r\ninner join MODALIDAD_PE mo on mo.ID_MODALIDAD_PE=pln.ID_MODALIDAD_PLANIFICACION\r\ninner join EMPLEADO pro on pro.IDENTIFICACION_EMP=pln.DNI_PROFESORC\r\ninner join MODALIDAD_PERIODO mope on per.ID_MODALIDAD=mope.ID_MODALIDAD\r\ninner join ESPACIOS_FISICOS ef on ef.ID_ESPACIOS_FISICOS=pln.ID_ESPACIOS_FISICOS inner join NIVEL_INFRAESTRUCTURA ninf on ninf.ID_NIVEL_INFRAESTRUCTURA=ef.ID_NIVEL_INFRAESTRUCTURA\r\ninner join INFRAESTRUCTURA inf on inf.ID_INFRAESTRUCTURA=ninf.ID_INFRAESTRUCTURA \r\ninner join SUBTIPO_COMPONENTE scpt ON pln.ID_TIPO_COMPONENTE = scpt.ID_SUBTIPO_COMPONENTE \r\ninner join ZEUS_NEW.dbo.UNIDAD_ORGANIZACION_CURRICULAR uoc on mat.ID_UOC = uoc.ID_UOC",
                "pln.ID_MALLA,\r\ncpt.HORAS_COMPONENTE as HORAS_SEMESTRALES_MATERIA,ROUND(CAST(cpt.HORAS_COMPONENTE AS FLOAT)/16,0) AS HORAS_SEMANALES_MATERIA,\r\nmat.CREDITOS_MATERIA,\r\nper.ID_PERIODO,\r\nper.CODIGO_PERIODO,\r\nmat.ID_MATERIA,\r\nmat.CODIGO_MATERIA,\r\nmat.NOMBRE_MATERIA,mat.ID_UOC,\r\nuoc.NOMBRE_UOC,\r\nPARALELO,CUPO,\r\npln.DNI_PROFESORC,\r\npro.NOMBRES_EMP,\r\npro.APELLIDO_EMP,\r\npln.ID_MODALIDAD_PLANIFICACION,\r\nmo.NOMBRE_MODALIDAD_PE,\r\nmope.NOMBRE_MODALIDADP,\r\nef.CODIGO_ESPACIOS_FISICOS, \r\nple.ID_PLAN_ESTUDIO,\r\nple.CODIGO_PLAN_ESTUDIO_MALLA,\r\nID_PLANIFICACION,\r\npln.ID_TIPO_COMPONENTE,\r\npln.ID_PERIODICIDAD_PLANIFICACION,\r\nef.ID_ESPACIOS_FISICOS,\r\nper.ID_MODALIDAD,\r\nper.ID_ESTADO_PERIODO,\r\nef.ID_NIVEL_INFRAESTRUCTURA,\r\ninf.ID_INFRAESTRUCTURA,\r\nscpt.CODIGO_SUBTIPO_COMPONENTE,\r\npln.ACTIVO,\r\npln.PROFESOR_S",
                "where pln.ID_PERIODO="+idperiodo+" and carr.ID_FACULTAD=" + idFacultad);
            if (ds_planificacion.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds_planificacion.Tables[0].Rows)
                {
                    ...
                    planificacion.idMalla = Convert.ToInt32(row["ID_MALLA"].ToString());
                    planificacion.horasSemestralesMateria = Convert.ToInt32(row["HORAS_SEMESTRALES_MATERIA"].ToString());
                    planificacion.horasSemanalesMateria = Convert.ToInt32(row["HORAS_SEMANALES_MATERIA"].ToString());
                    planificacion.creditosMateria = float.Parse(row["CREDITOS_MATERIA"].ToString());
                    planificacion.idPlanificacion = Convert.ToInt32(row["ID_PLANIFICACION"].ToString());
                    planificacion.idPeriodo = Convert.ToInt32(row["ID_PERIODO"].ToString());
                    planificacion.codigoPeriodo = row["CODIGO_PERIODO"].ToString();
                    ...
                    planificacion.codigoMateria = row["CODIGO_MATERIA"].ToString();
                    planificacion.nombreMateria = row["NOMBRE_MATERIA"].ToString();
                    planificacion.idUOC = Convert.ToInt32(row["ID_UOC"].ToString());
                    planificacion.nombreUOC = row["NOMBRE_UOC"].ToString();
                    planificacion.paralelo = row["PARALELO"].ToString();
                    planificacion.cupo = Convert.ToInt32(row["CUPO"].ToString());
                    planificacion.dniProfesorc = row["DNI_PROFESORC"].ToString();
                    planificacion.nombresEmp = row["NOMBRES_EMP"].ToString();
                    ...
                    planificacion.CodigoSubtipoComponente = row["CODIGO_SUBTIPO_COMPONENTE"].ToString();
                    planificacion.activo = Convert.ToBoolean(row["ACTIVO"].ToString());
                    planificacion.profesorS = Convert.ToBoolean(row["PROFESOR_S"].ToString());
                    planificacion.CODIGO_PLAN_ESTUDIO_MALLA = row["CODIGO_PLAN_ESTUDIO_MALLA"].ToString();
                    listaPlanificacion.Add(planificacion);
                    planificacion = new ComponentesPlanificacionDto();
                }
        ...
        public List<ComponentesPlanificacionDto> getPlanificacionSimultaneo(int idperiodo, int idfacultad,int idcarrera,int profesors,int idplanestudio, int idmodalidadplanificacio)
        {
            ComponentesPlanificacionDto planificacion = new ComponentesPlanificacionDto();
            List<ComponentesPlanificacionDto> listaPlanificacion = new List<ComponentesPlanificacionDto>();
            DataSet ds_planificacion = Conexion.BuscarZEUS_ds("PLANIFICACION pln inner join PERIODO per on per.ID_PERIODO=pln.ID_PERIODO inner join MALLA m on m.ID_MALLA = pln.ID_MALLA inner join COMPONENTE cpt on pln.ID_TIPO_COMPONENTE=cpt.ID_SUBTIPO_COMPONENTE and cpt.ID_MATERIA = m.ID_MATERIA and cpt.ID_PLAN_ESTUDIO = m.ID_PLAN_ESTUDIO \r\ninner join MATERIA mat on cpt.ID_MATERIA=mat.ID_MATERIA\r\ninner join PLAN_ESTUDIO ple on cpt.ID_PLAN_ESTUDIO=ple.ID_PLAN_ESTUDIO\r\ninner join MODALIDAD_PE mo on mo.ID_MODALIDAD_PE=pln.ID_MODALIDAD_PLANIFICACION\r\ninner join EMPLEADO pro on pro.IDENTIFICACION_EMP=pln.DNI_PROFESORC\r\ninner join MODALIDAD_PERIODO mope on per.ID_MODALIDAD=mope.ID_MODALIDAD\r\ninner join ESPACIOS_FISICOS ef on ef.ID_ESPACIOS_FISICOS=pln.ID_ESPACIOS_FISICOS inner join NIVEL_INFRAESTRUCTURA ninf on ninf.ID_NIVEL_INFRAESTRUCTURA=ef.ID_NIVEL_INFRAESTRUCTURA\r\ninner join INFRAESTRUCTURA inf on inf.ID_INFRAESTRUCTURA=ninf.ID_INFRAESTRUCTURA \r\ninner join SUBTIPO_COMPONENTE scpt ON pln.ID_TIPO_COMPONENTE = scpt.ID_SUBTIPO_COMPONENTE inner join CARRERA car on CAR.ID_CARRERA = PLE.ID_CARRERA\r\nINNER JOIN FACULTAD FAC ON FAC.ID_FACULTAD = CAR.ID_FACULTAD ", "pln.FECHA_INICIO_PLANIFICACION,pln.FECHA_FIN_PLANIFICACION,pln.ID_MALLA,\r\nmat.HORAS_SEMESTRALES_MATERIA,\r\nmat.CREDITOS_MATERIA,\r\nper.ID_PERIODO,\r\nper.CODIGO_PERIODO,\r\nmat.ID_MATERIA,\r\nmat.CODIGO_MATERIA,\r\nmat.NOMBRE_MATERIA,\r\nPARALELO,CUPO,\r\npln.DNI_PROFESORC,\r\npro.NOMBRES_EMP,\r\npro.APELLIDO_EMP,\r\npln.ID_MODALIDAD_PLANIFICACION,\r\nmo.NOMBRE_MODALIDAD_PE,\r\nmope.NOMBRE_MODALIDADP,\r\nef.CODIGO_ESPACIOS_FISICOS, \r\nple.ID_PLAN_ESTUDIO,\r\nple.CODIGO_PLAN_ESTUDIO_MALLA,\r\nID_PLANIFICACION,\r\npln.ID_TIPO_COMPONENTE,\r\npln.ID_PERIODICIDAD_PLANIFICACION,\r\nef.ID_ESPACIOS_FISICOS,\r\nper.ID_MODALIDAD,\r\nper.ID_ESTADO_PERIODO,\r\nef.ID_NIVEL_INFRAESTRUCTURA,\r\ninf.ID_INFRAESTRUCTURA,\r\nscpt.CODIGO_SUBTIPO_COMPONENTE,\r\npln.ACTIVO   ", "where pln.ID_PERIODO=" + idperiodo + " and FAC.ID_FACULTAD = case when " + idfacultad+ "=0 then fac.id_facultad else "+ idfacultad + " end and  car.ID_CARRERA = case when "+idcarrera+"=0 then car.ID_CARRERA else "+ idcarrera + " end AND PLN.PROFESOR_S="+profesors+"\r\nand ple.ID_PLAN_ESTUDIO = case when "+idplanestudio+"=0 then ple.ID_PLAN_ESTUDIO else "+ idplanestudio + " end and pln.ID_MODALIDAD_PLANIFICACION = case when "+idmodalidadplanificacio+"=0 then pln.ID_MODALIDAD_PLANIFICACION else "+ idmodalidadplanificacio + " end");
            if (ds_planificacion.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds_planificacion.Tables[0].Rows)
                {
        ...
        public List<ReportePlanificacionDto> getReportePlanificacionHorario(int idperiodo, int idfacultad, int idcarrera, int idplanestudio, string tipoPlanificacion)
        {
            ReportePlanificacionDto planificacion = new ReportePlanificacionDto();
            List<ReportePlanificacionDto> listaPlanificacion = new List<ReportePlanificacionDto>();
            DataSet ds_planificacion = Conexion.ExecZeusCore("[sp_ReportePlanificacion]", "'H'," + idperiodo + "," + idfacultad + "," + idcarrera + "," + idplanestudio + ",'" + tipoPlanificacion + "'");
            if (ds_planificacion.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds_planificacion.Tables[0].Rows)
                {
                    ...
                    planificacion.IdEspaciosFisicos = Convert.ToInt32(row["ID_ESPACIOS_FISICOS"].ToString());
                    planificacion.CodigoEspaciosFisicos = row["CODIGO_ESPACIOS_FISICOS"].ToString();
                    planificacion.AulaCapacidad = Convert.ToInt32(row["AULA_CAPACIDAD"].ToString());
                    planificacion.PDia = row["P_DIA"].ToString();
                    planificacion.PHIni = row["P_H_INI"].ToString();
                    planificacion.PHFin = row["P_H_FIN"].ToString();
                    planificacion.MFecha = row["M_FECHA"].ToString();
                    planificacion.MHIni = row["M_H_INI"].ToString();
                    planificacion.MHFin = row["M_H_FIN"].ToString();
